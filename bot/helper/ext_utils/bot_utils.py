        ...
        for index, download in enumerate(list(download_dict.values())[COUNT:], start=1):
            ...
            if download.status() not in [
                ...
                try:
                    ...
                    pass
                msg += f"<b>├⌬ 🚫Tᴏ Cᴀɴᴄᴇʟ→<code>/{BotCommands.CancelMirror} {download.gid()}</code>"
                ...
                msg += f"\n<code><b>├⌬ 🚫Tᴏ Cᴀɴᴄᴇʟ→/{BotCommands.CancelMirror} {download.gid()}</code>"
            else:
                msg += f"\n<b>├⌬ Size</b>{download.size()}"
            msg += "\n\n"
            if STATUS_LIMIT is not None and index == STATUS_LIMIT:
                break
        bmsg = f"<b>🖥️CPU→</b> {cpu_percent()}% | <b>⚗️FREE→</b> {get_readable_file_size(disk_usage(DOWNLOAD_DIR).free)}"
